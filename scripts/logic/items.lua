...
function metroid()
    local value = Tracker:ProviderCountForCode('metroid')
    if AUTOTRACKER_ENABLE_DEBUG_LOGGING_ITEMS then
        print(string.format("called gravity: value: %s",value))
    end
    if value > 0 then
...
function gravity()
    local value = Tracker:ProviderCountForCode('gravityboots')
    if AUTOTRACKER_ENABLE_DEBUG_LOGGING_ITEMS then
...
function spaceboost()
    local value = Tracker:ProviderCountForCode('spaceboost')
    if AUTOTRACKER_ENABLE_DEBUG_LOGGING_ITEMS then
        print(string.format("called spaceboost: value: %s",value))
    end
    if value > 0 then
        return 1
    end
...
function darkvisor()
    local value = Tracker:ProviderCountForCode('visor')
    if AUTOTRACKER_ENABLE_DEBUG_LOGGING_ITEMS then
